...
const WebSearchSection = ({ data }) => {
  ...
  // Aggregate all evidence into a flat array for cards
  const allEvidence = [];
  let sourceCounts = {
    'Official Website': 0,
    'Research Report': 0,
  ...
  categories.forEach(cat => {
    ...
    items.forEach(item => {
      ...
      // Determine source type logically from URL
      let sourceType = 'Official Website';
      let SourceIcon = Globe;
      const urlLower = item.url.toLowerCase();
      
      if (urlLower.includes('.gov') || urlLower.includes('.edu') || urlLower.includes('research') || urlLower.includes('statista') || urlLower.includes('gartner') || urlLower.includes('forrester') || urlLower.includes('mckinsey')) {
        sourceType = 'Research Report';
        SourceIcon = FileText;
      } else if (urlLower.includes('news') || urlLower.includes('techcrunch') || urlLower.includes('forbes') || urlLower.includes('bloomberg') || urlLower.includes('wsj')) {
        sourceType = 'News';
        SourceIcon = Newspaper;
      } else if (urlLower.includes('reddit') || urlLower.includes('quora') || urlLower.includes('forum') || urlLower.includes('community')) {
  ...
  // Deduplicate and limit to prevent overwhelming the UI
  const displayEvidence = allEvidence.slice(0, 12);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 border-b border-border/50 pb-4">
        ...
        <h2 className="text-2xl font-bold text-white tracking-tight">Research Evidence</h2>
      </div>
      ...
      {totalEvidence === 0 ? (
        <div className="glass-panel p-10 rounded-3xl border-border/50 flex flex-col items-center justify-center text-center space-y-5 shadow-lg bg-surface/30">
          ...
          <h3 className="text-xl md:text-2xl font-bold text-white tracking-tight">No external evidence was available for this analysis.</h3>
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
            <div className="glass-panel p-5 rounded-2xl border-border/50 shadow-lg relative overflow-hidden group hover:border-primary/30 transition-colors">
              ...
              <div className="text-3xl font-black text-white tracking-tight relative z-10">{totalEvidence}</div>
            </div>
            ...
            <div className="glass-panel p-5 rounded-2xl border-border/50 shadow-lg relative overflow-hidden group hover:border-success/30 transition-colors">
              ...
              <div className="text-3xl font-black text-white tracking-tight relative z-10">{sourceCounts['Official Website']}</div>
            </div>
            ...
            <div className="glass-panel p-5 rounded-2xl border-border/50 shadow-lg relative overflow-hidden group hover:border-info/30 transition-colors">
              ...
              <div className="text-3xl font-black text-white tracking-tight relative z-10">{sourceCounts['Research Report']}</div>
            </div>
            ...
            <div className="glass-panel p-5 rounded-2xl border-border/50 shadow-lg relative overflow-hidden group hover:border-warning/30 transition-colors">
              ...
              <div className="text-3xl font-black text-white tracking-tight relative z-10">{sourceCounts['News']}</div>
            </div>
            ...
            <div className="glass-panel p-5 rounded-2xl border-border/50 shadow-lg relative overflow-hidden group hover:border-purple-400/30 transition-colors">
              ...
              <div className="text-3xl font-black text-white tracking-tight relative z-10">{sourceCounts['Community']}</div>
            </div>
            ...
            <div className="glass-panel p-5 rounded-2xl border-border/50 shadow-lg relative overflow-hidden group hover:border-primary/30 transition-colors">
              ...
              <div className="text-3xl font-black text-white tracking-tight relative z-10">{categories.length}</div>
            </div>
          </div>
          ...
          <div className="space-y-6 pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {displayEvidence.map((evidence, i) => (
                <div key={i} className="glass-panel rounded-3xl border-border/50 shadow-lg overflow-hidden flex flex-col group hover:border-primary/40 transition-colors bg-gradient-to-br from-surface to-background/50">
                  ...
                  <div className="p-6 border-b border-border/30 flex justify-between items-start gap-4">
                    <div className="flex flex-col">
                      ...
                      <h4 className="text-sm font-bold text-white line-clamp-2 leading-relaxed group-hover:text-primary transition-colors">
                        {evidence.title}
                      </h4>
                    </div>
                  ...
                  <div className="p-4 bg-surface/30 border-t border-border/30 flex items-center justify-between">
                    ...
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer"
                      className="p-2 rounded-xl bg-primary/10 text-primary hover:bg-primary hover:text-white transition-colors shadow-sm"
                      title="Open Source"
                    >
